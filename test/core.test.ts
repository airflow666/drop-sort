...
group('Подсказка', () => {
  ...
  test('тупик без исчерпанных ходов помечен как deadEnd', () => {
    // Ходы ещё есть (витрина 3 пуста), но расклад не решить ни за один
    // порог до MAX_THRESHOLD — ровно баг из отчёта: без deadEnd подсказка
    // на такой доске зацикливает игрока между двумя перекладками.
    const b = new Board(level([[1, 0, 1], [0, 2, 1], [0, 2, 2], []], 3, 3));
    assert.equal(b.isDeadlock, false, 'ход формально доступен');
    const hint = findHint(b);
    assert.ok(hint, 'ход всё ещё предлагается — эвристический');
    assert.equal(hint.optimal, false);
